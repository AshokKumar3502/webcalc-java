    ...
    stages {
        stage('Checkout') {
            steps {
                ...
                git branch: 'main', url: 'https://github.com/AshokKumar3502/webcalc-java.git'
            }
        }
        ...
        stage('Deploy to Tomcat') {
            steps {
                ...
                deploy adapters: [tomcat8(credentialsId: 'tomcat', path: '', url: 'http://192.168.60.136:8081/')], contextPath: null, war: '**/*.war'
            }
        }
          ...
          //  steps {
            //    script {
              ...
              //      sleep(time: 30, unit: 'SECONDS')
                ...
                //    def getResponse = sh(script: 'curl -X GET http://192.60.136.114:8081/webapp-0.2/', returnStdout: true).trim()
                  //  def postResponse = sh(script: 'curl -X POST -d "n1=5&n2=6&r1=add" http://192.168.60.136:8081/webapp-0.2/firstHomePage', returnStdout: true).trim()
